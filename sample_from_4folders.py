...
# Ambil tepat N file acak per kelas dari dataset 4-folder dan salin/hardlink ke folder tujuan.
...
def copy_or_link(src: Path, dst: Path):
    ...
    try:
        if not dst.exists():
            os.link(src, dst)  # hemat disk (hardlink; harus di drive yang sama)
    except Exception:
